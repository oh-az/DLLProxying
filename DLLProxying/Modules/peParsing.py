...
def parse():
	...
	for export in allExports:
		print(Fore.GREEN + '#pragma comment(linker, "/export:'+ str(export).replace("b","").replace("'","") + '=\\"C:\\\\Windows\\\\' + str(check.directory) +'\\\\' + str(check.fileName.replace(".dll","").split("/")[-1]) + "." + str(export).replace("b","").replace("'","") +'\\"")')
	print("\n")
